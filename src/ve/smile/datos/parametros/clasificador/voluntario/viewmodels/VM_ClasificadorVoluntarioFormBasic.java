...
public class VM_ClasificadorVoluntarioFormBasic extends VM_WindowForm {
	...
	public boolean actionGuardar(OperacionEnum operacionEnum) {
		...
		if (operacionEnum.equals(OperacionEnum.INCLUIR)) {
			PayloadClasificadorVoluntarioResponse payloadClasificadorVoluntarioResponse = S.ClasificadorVoluntarioService
					.incluir(getClasificadorVoluntario());
		...
		if (operacionEnum.equals(OperacionEnum.MODIFICAR)) {
			PayloadClasificadorVoluntarioResponse payloadClasificadorVoluntarioResponse = S.ClasificadorVoluntarioService
					.modificar(getClasificadorVoluntario());
			Alert.showMessage(payloadClasificadorVoluntarioResponse);
			if (!UtilPayload.isOK(payloadClasificadorVoluntarioResponse)) {
				Alert.showMessage(payloadClasificadorVoluntarioResponse);
				return true;
	...
	public boolean isFormValidated() {
		try {
			UtilValidate.validateString(
					getClasificadorVoluntario().getNombre(), "Nombre", 100);
			UtilValidate.validateString(getClasificadorVoluntario()
